  ...
  return (
    <ToolPageLayout title="PDF → Excel" description="AI-powered table and data extraction into clean spreadsheets.">
      ...
      <ProgressBar
        active={isProcessing}
        complete={isComplete}
        stages={['Uploading PDF', 'Detecting tables', 'Extracting data', 'Building spreadsheet']}
      />
      {showResult && state.result && (
        <ResultBox title="Extracted" conversion="PDF → Excel" subtitle="Your Excel file is ready" downloadUrl={state.result.url} downloadName={state.result.filename || 'forma-export.xlsx'} onReset={handleReset} />
      )}
      {state.status === 'error' && (
        <div>
